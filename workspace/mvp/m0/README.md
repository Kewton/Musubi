...
- [x] M0-1〜M0-4 の DoD がすべて green（各 Issue のクローズと受入試験 A〜F で確認。`05` §7 の宣言）
- [x] §5 の事前宣言7項目を実測し、清算表に記録した（未達があれば処置を明記）。**課金額が $0 であることを含む**（`05` §6）
- [x] Milestone `M0` の Issue がすべて closed（持ち越しは `M1` に付け替え）
- [x] `docs/runbook/rollback.md` が存在し、**実際に1回ロールバックを実演した記録がある**（§4.2）

→ 判定後、直ちに **M1（垂直スライス「工場→店頭」）** へ。M1のゲートは「封緘済みgolden割り勘がstagingのスマホで動く」。判定方法・判定者・期限は `05` §7 の宣言（2026-09-15）。
