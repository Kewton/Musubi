...
- [x] 🤖 試験A：dev destroy→apply→sync→deploy→smoke（**≤15分・手作業0回**）— 32 秒・0 回
- [x] 🤖 試験B：Service Bindings 結線＋**越境importでCIが落ちること**を実証 — PR #68
- [x] 🧑🤖 試験C：CI/CD 一周（C-1〜C-9）— C-1 は試さず API で確認（`05` §3）
- [x] 🤖 試験D：Issueドリブン運用の確認 — 全PRの Closes が未達。処置は `05` §6
- [x] 🤖 試験E：ドキュメント6点の実在確認
- [x] 🤖 **試験F：無償枠の実測**（CPU独立/合算・SBのリクエストカウント・静的アセット・課金 $0）— `06` §8
- [x] 🧑 **清算表を記入**（宣言 vs 実測・未達の処置・帰属分析）— `05` §6
- [x] 🧑 Milestone `M0` を close ／ `v0.1.0` を production へ — 2026-09-15 / 2026-09-14
- [x] 🧑 **M1のゲートを事前宣言**（「封緘済みgolden割り勘が staging のスマホで動く」の判定方法・判定者・期限）— `05` §7
