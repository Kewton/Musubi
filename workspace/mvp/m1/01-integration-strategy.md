...
> 状態：**進め方は決定**（2026-09-15）。D2〜D7 は所有者が決めた（§6）。
> **D1（サンプルの形）は、具体例 [`02-l2-spec-examples.md`](./02-l2-spec-examples.md) を見て決める。** M1a と M1b のゲートは、§5.3 の叩き台を見て所有者が宣言する。
...
|---|---|---|---|---|
| **1. 見本を動かす** | 店頭がアプリを動かせることを示す | 手書きの見本 3 つ（割り勘・タスク管理・ダッシュボード）・スキーマの草案・評価器・API・DO の表・画面・publish | このリポジトリ | 使わない |
...
### 5.3 ゲートの叩き台（所有者が宣言する）

> ゲートの宣言は所有者のもの（`../m0/05-acceptance.md` §7）。**この節は叩き台であり、宣言はまだ変えていない。**
> 所有者が宣言したら、日付と名前を入れて、README の §1.2 と `00-open-questions.md` に反映する。
...
| 判定方法 | 1. 3 つの見本の採点のシナリオ（`02` の §2.3・§3.3・§4.4）を e2e テストで流し、値が期待値と一致する（時計を固定する。機械で確認）<br/>2. スマホ 1 台で staging の URL を開き（OS とブラウザを記録）、割り勘のシナリオを入力して「C さん → A さん 3,000 円」が出る。タスクを「完了にする」と列が移る。今日の日付で活動を記録すると「今月の活動」が増える<br/>3. 再読み込み・別端末でもデータが残る（DO）<br/>4. 3 つの見本がスキーマの新しい版の検査に通り、負例は落ちる（CI で機械確認）<br/>5. staging に置かれた見本の SHA-256 が、リポジトリの見本と一致する（機械で確認）<br/>6. スキーマの正本の SHA-256 が `pins/` に記録され、工場へ渡す値として確定している |
| 判定者 | Kewton |
| 期限 | Issue を切り終えた時点で、Issue の量を見て宣言する（目安：着手から 4 週間） |
...
|---|---|---|---|---|
| D1 | サンプルアプリの形 | **L2 の宣言（`app.spec.yaml`）を手で書く** | MVP のアプリは L2。GUI から数分で作る体験もビルドの無い L2 が前提。TanStack Start のアプリを手で作ると、将来の生成物と形が違って見本にならない | **未定**。具体例（`02`）を見て決める |
...
| D5 | マイルストーンの分け方 | **案 A（M1a と M1b に分ける）** | §5.2 | **分ける。M1a はこのリポジトリで完結、M1b は CommandAgent との結合**。ゲートは §5.3 の叩き台を見て宣言する |
...
この進め方にすると、問いのいくつかは中身が変わる。**D1 と M1a・M1b のゲートの宣言が済んだら、問いを見直す。**
...
| 精算や集計を入れると計算が増え、CPU 10 ms に効く | 段階 1 で staging の CPU 時間を実測する（`README.md` §7.2）。見本ごとに、データが多いときも測る |
| 見本が 3 つになり、M1a の作業が膨らむ（画面の部品だけで `list`・`table`・`settlement`・`board`・`dashboard` の 5 種類） | 割り勘で経路を全部通してから、タスク管理・ダッシュボードの部品を足す。Issue を切った時点で量を見て、M1a の期限を宣言する |
| 時刻で変わる値（期限切れ・今月）は、採点のたびに結果が変わる | 採点では時計を固定する。「今日」をどの時間帯で数えるかを、実行時の意味として決める（`02` §6.1） |
| 「ダッシュボード」の意味が、所有者の想定と違う | `02` §4.1 の前提（このアプリに入れたデータを集計する）を所有者が確かめてから、見本を書く |
