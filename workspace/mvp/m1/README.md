...
>
> ⚠️ **進め方の見直し中（2026-09-15）**：所有者の方針「まず手書きのサンプルアプリを Cloudflare で動かし、その後に CommandAgent を合わせて結合する」を受けて、
> 進め方の案を [`01-integration-strategy.md`](./01-integration-strategy.md) にまとめた。**§1 以降は、工場の golden を先に動かす前提で書かれている。**
> 進め方が決まったら、§1 以降と `00-open-questions.md` を見直す。
...
| 2〜3 | 指示を受け付ける GUI、工場の自動起動、進み具合の通知（連携 Lv2） | M2（M1 では、開発者が手で工場を動かすか、手書きの見本を置く） |
| 4 | 納品物を置いて登録する仕組み（publish）、宣言から画面を出す仕組み（Instant Renderer） | **M1** |
| 5〜6 | 招待リンク、ゲスト参加（名前を選んで参加） | M2 |
| 7 | 入力の検証と保存（data-api・DO） | **M1**（他の人の画面にすぐ出るリアルタイム同期は M2） |
| 8 | 精算（誰が誰へいくら） | **M1 の見本で入れるかを決める**（v0.1 では書けない。`01-integration-strategy.md` の D4） |
