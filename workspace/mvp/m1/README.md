...
| M1-2 | 契約 | スキーマの新しい版・実行時の意味の文書・正例と負例・静的チェックツールが揃い、スキーマの SHA-256 が `pins/` に記録されている | **M1a** |
...
- 判定者：Kewton
- 期限：M1a の Issue を切り終えた時点で、Issue の量を見て宣言する（目安：着手から 4 週間。Q10）
...
- そのうち**利用者の画面の数字に出るもの**は、採点のシナリオの期待値に効くので、着手前に決める（Q13）
...
| `pins/commandagent.json` | スキーマは v0.1（工場の仮置き）。テンプレの tarball は null（#38） | スキーマのピンを、このリポジトリの新しい版へ差し替える（Q9） | — |
| `packages/connector` | 中身なし（M6 まで空） | 触らない（データ連携層は M1 で作らない） | データ連携 |
| `templates/tanstack-start` | README のみ | 使わない（Q4） | — |
...
| ② | ピン差し替えの儀式 | このリポジトリ側は M1a で行う（スキーマのピンを新しい版へ。Q9）。M1b では、工場が同じ版のピンを持っていることを照合する | 工場側の差し替えの儀式を、CommandAgent のリポジトリの Issue で行う（D6） |
...
|---|---|---|---|
| R-1 | 見本が 3 つになり、M1a の作業が膨らむ（画面の部品 5 種類・集計・静的チェック） | M1a の期限に収まらない | 割り勘で経路を全部通してから、タスク管理・ダッシュボードの部品を足す（§10）。Issue を切った時点で量を見て、M1a の期限を宣言する |
...
| タイミング | 作業内容 |
|---|---|
| M1a の着手前 | `00-open-questions.md` の M1a の問いに答える。<br>M1a の Issue を起票した日を着手日とし、Issue の量を見て **M1a の期限を宣言する**（`01` §5.3） |
| M1a の実装中 | 割り勘が staging で動いた時点で、スマホで途中確認する（中間の区切り）。<br>利用者に見える計算の決めごとを変えるときは承認する（Q13）。<br>スキーマのピンの差し替えを承認する（Q9） |
...
この節で言いたいこと：
所有者の判断を受けて M1a の Issue を起票し、着手日と期限を決めて開発に入る。

1. 所有者が [`00-open-questions.md`](./00-open-questions.md) の M1a の問いに答える（おすすめの採用でよければその旨を合意する）
2. 回答を受けて、§10 の波ごとに依存関係を整理した Issue 一覧を作り、起票する
3. 起票した日を M1a の着手日とし、Issue の量を見て M1a の期限を宣言する（`01` §5.3）
