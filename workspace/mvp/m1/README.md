...
> | 文書 | 中身 |
> |---|---|
> | [`../roadmap.md`](../roadmap.md) | **ロードマップ**。M1 を M1.1〜M1.6 の「できること」に分けたマイルストーンの正本 |
> | [`00-open-questions.md`](./00-open-questions.md) | 着手前に所有者が決めること |
...
| **M1a（これから）** | **手書きの見本 3 つが staging のスマホで動く** | 所有者のスマートフォン（ログインなし） | **契約（スキーマの新しい版）を固め、ピンで渡せる状態にする** |
...
| M1-2 | 契約 | スキーマの新しい版・実行時の意味の文書・正例と負例・静的チェックツールが揃い、スキーマの SHA-256 が `pins/` に記録されている。**テンプレート（`templates/tanstack-start`）の中身と、再現できる tarball の SHA-256 も `pins/` に記録されている**（#38。Q4） | **M1a** |
...
1. 3 つの見本の採点のシナリオを e2e で流し、期待値と一致する（時計を固定。機械で確認）
2. スマホで staging を開き、割り勘の精算・タスクの列の移動・今月の活動の増加を確かめる
3. 再読み込み・別端末でもデータが残る（DO）
4. 見本がスキーマの新しい版の検査に通り、負例は落ちる（CI で機械確認）
5. staging に置かれた見本の SHA-256 が、リポジトリの見本と一致する（機械で確認）
6. スキーマの正本の SHA-256 が `pins/` に記録されている
...
この節で言いたいこと：
動かす対象は、手で書いた L2 の見本 3 つである。3 つは今のスキーマ v0.1 では書けないので、見本を動かしながらスキーマの新しい版を作る。
...
- 見本を動かしながら MUSUNEST が決め、スキーマの新しい版と一緒に `appspec-schema` に文書で持つ（Q3）
...
この節で言いたいこと：
M1a では、手書きの見本を静的チェックに通してから R2 / D1 に置き、Cloudflare 上の Worker 群を経由してスマートフォンの画面へ届ける。M1b では、見本の代わりに工場の納品物を同じ経路に流す。

```mermaid
flowchart LR
  subgraph REPO["このリポジトリ（手元・CI）"]
    SAMPLE["手書きの見本<br/>app.spec.yaml ×3・負例"] --> CHECK["静的チェック<br/>（spec-engine）"]
    CHECK --> PUB["publish（M1a で作成）<br/>検査済みの形を作って置く"]
    PINS["pins/<br/>スキーマの新しい版の SHA"]
    E2E["e2e<br/>採点のシナリオ（時計を固定）"]
  end
...
|---|---|---|---|
| `packages/appspec-schema` | バージョン定数のみ（`APPSPEC_SCHEMA_VERSION = "0.0.0-m0"`） | スキーマの新しい版（草案 → 正本）、型定義、実行時の意味の文書、層の定義 | 全体 |
| `packages/spec-engine` | 中身なし | **静的チェック**（形・データ層・ロジック層の式・層の向き・画面の参照・上限。誤りコードつき）と、式・集計・店頭が用意する関数（`sum`・`count`・`today`・`settle` など）の評価。**評価の前に静的チェックを通す** | データ・ロジック |
| 静的チェックの入口 | 無い | 手元・CI・publish から呼ぶコマンド（置き場所は最初の Issue で決める） | — |
| `packages/app-do` | healthz 用の `kv` 表のみ | 宣言の entity のレコードを持つ表。1 インスタンス 1 DO（クラス名 `AppInstanceDO` は変えない） | データ |
| `packages/control-plane` | メタ表（`_musunest_meta`）のみ | アプリ（宣言の SHA-256）とインスタンスを登録する D1 マイグレーション（前方互換の規律どおり） | — |
| `packages/data-api` | `/healthz` のみ | 宣言の取得（R2）、レコードの作成・一覧・更新・削除（検査と操作の条件つき）、computed と集計の計算、「操作できるか」の判定を返す API | ロジック・権限 |
| `apps/gateway` | `/healthz` の中継のみ | `/api/...` の中継（M1 は認可なし。M2 で AppGrant 検査を追加） | — |
| `apps/host` | `/api/*` は 404 応答 | `/api/*` の中継、**Instant Renderer**（フォーム、`list`・`table`・精算・`board`・`dashboard` の部品、最小の画面遷移）、UX の部品（絞り込み・強調・削除の確認） | UI・UX |
| `packages/sdk` | 中身なし | host から data-api を呼ぶ型付きクライアント | — |
| 見本 | 無い | 見本 3 つの宣言と負例（このリポジトリに置く。D7。置き場所は最初の Issue で決める） | — |
| `e2e` | 中身なし | 採点のシナリオ 3 つ（時計を固定）、staging に置かれた見本の SHA-256 の照合 | — |
| `pins/commandagent.json` | スキーマは v0.1（工場の仮置き）。テンプレの tarball は null（#38） | スキーマのピンを、このリポジトリの新しい版へ差し替える（Q9）。テンプレの tarball の実値を入れる（#38） | — |
...
| `infra/scripts` | deploy・smoke・計測 | `publish`（静的チェック → R2 / D1） | — |
| 計測 | health chain の CPU 時間 | 宣言の読み込み・計算・集計の CPU 時間を staging で実測する | — |

- **依存の向き**：今の `infra/scripts/dep-graph.mjs` の許可で作れる。data-api は `appspec-schema`・`sdk`・`spec-engine`・`app-do` を参照でき、host は `sdk` だけを参照する。静的チェックの入口の置き場所によって依存の向きを変えるときは、`dep-graph.mjs` を直す（`CLAUDE.md`）
...
| ② | ピン差し替えの儀式 | このリポジトリ側は M1a で行う（スキーマのピンを新しい版へ。Q9。テンプレの tarball のピンも M1a で入れる）。M1b では、工場が同じ版のピンを持っていることを照合する | 工場側の差し替えの儀式（スキーマと、合成テンプレから本物のテンプレへ）を、CommandAgent のリポジトリの Issue で行う（D6） |
...
進め方を変える前は、この納品物を M1 のゲートに使う予定だった。**今の M1a・M1b のゲートには使わない**（Q1）。v0.1 と新しい版の違いを確かめる材料として、記録を残す。
...
|---|---|---|---|
| R-1 | 見本が 3 つとテンプレートで、M1a の作業が膨らむ（画面の部品 5 種類・集計・静的チェック・テンプレート） | M1a がいつまでも終わらない | 割り勘で経路を全部通してから、タスク管理・ダッシュボードの部品を足す（§10）。期限は設けないので、波 4 の途中確認で進み具合を見て、残りの量を見直す |
| R-2 | 見本に合わせすぎて、工場が作りにくい契約になる | M1b で工場の調整が長引く | 段階 2 で、工場側の契約文書と検査の仕組みに照らしてから固める |
| R-3 | 処理時間が CPU 10 ms の上限を超える（§7.2） | staging で動いても production の昇格トリガーに触れる | M1a の中で CPU 時間を実測する。超えたら即座に $5 プランへ昇格する |
| R-4 | 認証なしの API が production に露出する（§7.3） | 誰でも production の DO にデータを書き込めてしまう | production では 404 を返す（Q5） |
| R-5 | 時刻で変わる値（期限切れ・今月）の採点が、実行のたびに変わる | e2e が不安定になり、ゲート判定 1 が揺れる | 採点では時計を固定する。「今日」の数え方を着手前に決める（Q13） |
...
| R-8 | orchestrate の実行契約上限（ソース約 30 本・`.tf` 変更不可）に抵触する | 大きすぎる Issue を自動 dispatch できない | §10 の方針に従い、適切な粒度に Issue を分割する |
...
| タイミング | 作業内容 |
|---|---|
| M1a の着手前 | `00-open-questions.md` の M1a の問いに答える（2026-09-15 済み）。<br>M1a の Issue を起票した日を着手日として記録する（期限は設けない。Q10） |
| M1a の実装中 | 割り勘が staging で動いた時点で、スマホで途中確認し、残りの量を見直す（中間の区切り）。<br>利用者に見える計算の決めごとを変えるときは承認する（Q13）。<br>スキーマのピンの差し替えを承認する（Q9）。<br>テンプレートの配布形態（GitHub Releases のアセットか R2 か）を決める（#38） |
...
## 10. Issue の切り出し方針（M1a）

この節で言いたいこと：
Issue は 1 パッケージ前後・ソース 30 本程度までの単位で分割する。まず割り勘で経路を全部通し、次にタスク管理とダッシュボードの部品を足し、最後に契約を固める。

> **Issue はロードマップのマイルストーン（[`../roadmap.md`](../roadmap.md) §4）ごとに、「今」と「次」の分だけ切る。** 下の波は中身の目安であり、波 1〜4 は M1.1・M1.2、波 5 は M1.3、波 6 は M1.4、波 7 は M1.5 に当たる。

- **分割の基準**：
  - 1 Issue あたり 1 パッケージ前後、ソースコード 30 本程度を上限とする（orchestrate の実行契約の制約）
  - `.tf` を含む変更は監督側（人と Claude）が担当する（orchestrate の scope に載らないため）
- **実装順序（波）**：
  1. **波 1：土台（並列作業可）**：
     - `appspec-schema`：スキーマの新しい版の草案（割り勘に要る範囲）、型定義、層の定義
     - `spec-engine`：静的検査（形・データ層・ロジック層の式。誤りコードつき）と、式と集計の評価。静的チェックの入口
     - `app-do`：レコードを持つ表
     - `control-plane`：登録用の D1 マイグレーション
     - 見本 1（割り勘）の宣言と負例
  2. **波 2：経路（割り勘で通す）**：
     - `data-api`：宣言の取得、レコードの作成・一覧・更新・削除、計算値と「操作できるか」を返す API
     - `gateway` / `host`：API の中継
     - `publish` スクリプト（静的チェック → R2 / D1）
  3. **波 3：画面（割り勘）**：
     - `host`：Instant Renderer（フォーム・`list`・`table`・精算）
     - `sdk`：型付きクライアント
  4. **波 4：割り勘の証拠（中間の区切り）**：
     - e2e：割り勘の採点のシナリオ、見本の SHA-256 の照合
     - CPU 時間の実測
     - 🧑 スマホでの途中確認
  5. **波 5：タスク管理**：選択肢・日付・「今日」・状態の書き換え・操作の条件、`board` と絞り込み、見本 2、e2e
  6. **波 6：ダッシュボード**：アプリ全体の集計・月ごとの集計、`dashboard` の部品（数値・棒・円・順位）、見本 3、e2e
  7. **波 7：契約を固める**：
     - スキーマの新しい版の正本、実行時の意味の文書、正例と負例の整理
     - 静的チェックの残り（層の向き・画面の参照・上限）
     - `pins/` のスキーマの差し替え（Q9）
     - テンプレート（#38）：`templates/tanstack-start` の中身、再現できる tarball、配布形態、`pins/` の `template_tarball`
- **M1b の Issue**：M1a のゲートに合格してから切る。工場の調整は CommandAgent のリポジトリ、このリポジトリには §5.1 の ①④ と、見本と生成物を比べる e2e を切る
- **既存の M1 関連 Issue**：
  - #38（テンプレート tarball の SHA）：M1 に残す（Q4）。波 7 で扱う
...
この節で言いたいこと：
所有者の判断を受けて M1a の Issue を起票し、着手日を記録して開発に入る。

1. ~~所有者が [`00-open-questions.md`](./00-open-questions.md) の M1a の問いに答える~~（2026-09-15 済み）
2. §10 の波ごとに依存関係を整理した Issue 一覧を作り、起票する
3. 起票した日を M1a の着手日として記録する（期限は設けない。Q10）
